...
const GroceriesSlide = () => {
  ...
  useEffect(() => {
    const fetchLaptops = () => {
      const filteredGroceries = data.filter(
        (gro) => gro?.category === "groceries"
      );
  ...
  return (
    <div className="flex flex-col justify-center py-2  mx-auto w-full">
      <div>
        <Typography className="text-center text-xl md:text-2xl font-Poppins md:text-start py-2 font-bold md:ml-16">
      ...
      <div className="flex flex-row justify-center relative">
        <div
          onClick={scrollLeft}
          className="hidden md:inline-flex absolute left-1 z-[40]  top-40 bg-gray-500 p-3 rounded-full cursor-pointer"
        >
          <ArrowLeft />
        </div>
        <div
          id={"slider" + 5}
          className="flex flex-row overflow-x-scroll relative gap-4 py-2 px-4"
        >
          {groceries?.map((item, id) => (
            <div key={id} className="flex flex-row">
              <Card className="relative w-48 md:w-64 mx-auto hover:scale-105 cursor-pointer transition-all duration-300 delay-200 ease-in-out">
                <Typography className="absolute text-xs top-1 left-3 z-[40] bg-red-700 px-2  text-white">
                  {item?.subcategory}
                </Typography>
                <div
                  onClick={() =>
                ...
                <CardBody>
                  <div className="mb-2 flex flex-col items-center ">
                    ...
                    <div className="py-2 flex flex-col justify-start md:justify-center items-center">
                      <Typography
                        color="blue-gray"
                        className="text-xs md:text-sm md:font-medium font-Poppins"
                      >
                        {item?.name}
                      </Typography>
                      <Typography
        ...
        <div
          onClick={scrollRight}
          className="hidden md:inline-flex absolute right-1 z-[40] top-40 bg-gray-500 p-3 rounded-full cursor-pointer"
        >
          <ArrowRight />
        </div>
      </div>
