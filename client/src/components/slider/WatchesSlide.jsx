...
const WatchesSlide = () => {
    ...
    useEffect(() => {
        const fetchWatches = () => {
            const filteredWatches = data.filter((women) => women?.category === "watches")
            setWatches(filteredWatches)
        }
        fetchWatches()
    }, [])
    const scrollLeft = () => {
        let slider = document.getElementById('slider' + 6);
    ...
    return (
        <div className="flex flex-col justify-center py-2  mx-auto w-full">
            <div>
                <Typography className="text-center text-xl md:text-2xl font-Poppins md:text-start py-2 font-bold md:ml-16">Best Of watches</Typography>
            </div>
            <div className="flex flex-row  relative">
                ...
                <div id={"slider" + 6} className="flex flex-row overflow-x-scroll relative gap-4 py-2 px-4">
                    {watches?.map((item, id) =>
                        <div key={id} className="flex flex-row">
                            <Card className="relative w-48 md:w-64 mx-auto hover:scale-105 cursor-pointer transition-all duration-300 delay-200 ease-in-out" >
                                <Typography className="absolute text-xs top-1 left-3 z-[40] bg-red-700 px-2  text-white">{item?.brand}</Typography>
